...
```csharp
TaskrouterV1TaskReservationApi taskrouterV1TaskReservationApi = client.TaskrouterV1TaskReservationApi;
```
...
| `reservationStatus` | [`TaskReservationStatus?`](../../doc/models/task-reservation-status.md) | Query, Optional | Returns the list of reservations for a task with a specified ReservationStatus.  Can be: `pending`, `accepted`, `rejected`, or `timeout`. |
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `Data` property of this instance returns the response data which is of type [Models.ListTaskReservationResponse](../../doc/models/list-task-reservation-response.md).

## Example Usage

```csharp
string workspaceSid = "WorkspaceSid0";
string taskSid = "TaskSid8";
try
{
    ApiResponse<ListTaskReservationResponse> result = await taskrouterV1TaskReservationApi.ListTaskReservationAsync(
        workspaceSid,
        taskSid
    );
}
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `Data` property of this instance returns the response data which is of type [Models.TaskReservation](../../doc/models/task-reservation.md).
...
try
{
    ApiResponse<TaskReservation> result = await taskrouterV1TaskReservationApi.FetchTaskReservationAsync(
        workspaceSid,
        taskSid,
        sid
    );
...
| `reservationStatus` | [`TaskReservationStatus?`](../../doc/models/task-reservation-status.md) | Form, Optional | The current status of the reservation. Can be: `pending`, `accepted`, `rejected`, or `timeout`. |
...
| `statusCallbackMethod` | [`ConfigurationWebhookMethod?`](../../doc/models/configuration-webhook-method.md) | Form, Optional | The HTTP method we should use to call `status_callback`. Can be: `POST` or `GET` and the default is `POST`. |
| `statusCallbackEvent` | [`List<TaskReservationCallStatus>`](../../doc/models/task-reservation-call-status.md) | Form, Optional | The call progress events that we will send to `status_callback`. Can be: `initiated`, `ringing`, `answered`, or `completed`. |
...
| `waitMethod` | [`ConfigurationWebhookMethod?`](../../doc/models/configuration-webhook-method.md) | Form, Optional | The HTTP method we should use to call `wait_url`. Can be `GET` or `POST` and the default is `POST`. When using a static audio file, this should be `GET` so that we can cache the file. |
...
| `conferenceStatusCallbackMethod` | [`ConfigurationWebhookMethod?`](../../doc/models/configuration-webhook-method.md) | Form, Optional | The HTTP method we should use to call `conference_status_callback`. Can be: `GET` or `POST` and defaults to `POST`. |
| `conferenceStatusCallbackEvent` | [`List<TaskReservationConferenceEvent>`](../../doc/models/task-reservation-conference-event.md) | Form, Optional | The conference status events that we will send to `conference_status_callback`. Can be: `start`, `end`, `join`, `leave`, `mute`, `hold`, `speaker`. |
...
| `recordingStatusCallbackMethod` | [`ConfigurationWebhookMethod?`](../../doc/models/configuration-webhook-method.md) | Form, Optional | The HTTP method we should use when we call `recording_status_callback`. Can be: `GET` or `POST` and defaults to `POST`. |
...
| `conferenceRecordingStatusCallbackMethod` | [`ConfigurationWebhookMethod?`](../../doc/models/configuration-webhook-method.md) | Form, Optional | The HTTP method we should use to call `conference_recording_status_callback`. Can be: `GET` or `POST` and defaults to `POST`. |
...
| `dequeueStatusCallbackEvent` | `List<string>` | Form, Optional | The Call progress events sent via webhooks as a result of a Dequeue instruction. |
...
| `supervisorMode` | [`TaskReservationSupervisorMode?`](../../doc/models/task-reservation-supervisor-mode.md) | Form, Optional | - |
...
string taskSid = "TaskSid8";
string sid = "Sid8";
TaskReservationStatus? reservationStatus = TaskReservationStatus.Accepted;
string instruction = "supervise";
TaskReservationSupervisorMode? supervisorMode = TaskReservationSupervisorMode.Monitor;
string supervisor = "WKaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
try
{
    ApiResponse<TaskReservation> result = await taskrouterV1TaskReservationApi.UpdateTaskReservationAsync(
        workspaceSid,
        taskSid,
        sid,
        null,
