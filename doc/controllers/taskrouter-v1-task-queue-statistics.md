...
```csharp
TaskrouterV1TaskQueueStatisticsApi taskrouterV1TaskQueueStatisticsApi = client.TaskrouterV1TaskQueueStatisticsApi;
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `Data` property of this instance returns the response data which is of type [Models.TaskQueueStatistics](../../doc/models/task-queue-statistics.md).
...
try
{
    ApiResponse<TaskQueueStatistics> result = await taskrouterV1TaskQueueStatisticsApi.FetchTaskQueueStatisticsAsync(
        workspaceSid,
        taskQueueSid,
        endDate,
        minutes,
