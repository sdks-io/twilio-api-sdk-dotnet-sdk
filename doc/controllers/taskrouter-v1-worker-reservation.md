...
```csharp
TaskrouterV1WorkerReservationApi taskrouterV1WorkerReservationApi = client.TaskrouterV1WorkerReservationApi;
```
...
| `reservationStatus` | [`WorkerReservationStatus?`](../../doc/models/worker-reservation-status.md) | Query, Optional | Returns the list of reservations for a worker with a specified ReservationStatus. Can be: `pending`, `accepted`, `rejected`, `timeout`, `canceled`, or `rescinded`. |
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `Data` property of this instance returns the response data which is of type [Models.ListWorkerReservationResponse](../../doc/models/list-worker-reservation-response.md).
...
try
{
    ApiResponse<ListWorkerReservationResponse> result = await taskrouterV1WorkerReservationApi.ListWorkerReservationAsync(
        workspaceSid,
        workerSid
    );
}
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `Data` property of this instance returns the response data which is of type [Models.WorkerReservation](../../doc/models/worker-reservation.md).
...
try
{
    ApiResponse<WorkerReservation> result = await taskrouterV1WorkerReservationApi.FetchWorkerReservationAsync(
        workspaceSid,
        workerSid,
        sid
    );
...
| `reservationStatus` | [`WorkerReservationStatus?`](../../doc/models/worker-reservation-status.md) | Form, Optional | The current status of the reservation. Can be: `pending`, `accepted`, `rejected`, `timeout`, `canceled`, or `rescinded`. |
...
| `statusCallbackEvent` | [`List<WorkerReservationCallStatus>`](../../doc/models/worker-reservation-call-status.md) | Form, Optional | The call progress events that we will send to `status_callback`. Can be: `initiated`, `ringing`, `answered`, or `completed`. |
...
| `conferenceStatusCallbackEvent` | [`List<WorkerReservationConferenceEvent>`](../../doc/models/worker-reservation-conference-event.md) | Form, Optional | The conference status events that we will send to `conference_status_callback`. Can be: `start`, `end`, `join`, `leave`, `mute`, `hold`, `speaker`. |
...
string workerSid = "WorkerSid0";
string sid = "Sid8";
WorkerReservationStatus? reservationStatus = WorkerReservationStatus.Accepted;
try
{
    ApiResponse<WorkerReservation> result = await taskrouterV1WorkerReservationApi.UpdateWorkerReservationAsync(
        workspaceSid,
        workerSid,
        sid,
        null,
