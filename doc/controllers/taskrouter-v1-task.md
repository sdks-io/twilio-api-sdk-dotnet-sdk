...
```csharp
TaskrouterV1TaskApi taskrouterV1TaskApi = client.TaskrouterV1TaskApi;
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `Data` property of this instance returns the response data which is of type [Models.TaskRouterTask](../../doc/models/task-router-task.md).
...
try
{
    ApiResponse<TaskRouterTask> result = await taskrouterV1TaskApi.FetchTaskAsync(
        workspaceSid,
        sid
    );
}
...
| `assignmentStatus` | [`TaskStatus?`](../../doc/models/task-status.md) | Form, Optional | The current status of the Task's assignment. Can be: `pending`, `reserved`, `assigned`, `canceled`, `wrapping`, or `completed`. |
...
TaskStatus? assignmentStatus = TaskStatus.Pending;
string reason = "reason";
int? priority = 1;
DateTime? virtualStartTime = DateTime.ParseExact("08/02/2023 12:34:56", "yyyy'-'MM'-'dd'T'HH':'mm':'ss.FFFFFFFK",
        provider: CultureInfo.InvariantCulture,
        DateTimeStyles.RoundtripKind);
try
{
    ApiResponse<TaskRouterTask> result = await taskrouterV1TaskApi.UpdateTaskAsync(
        workspaceSid,
        sid,
        null,
        attributes,
...
try
{
    await taskrouterV1TaskApi.DeleteTaskAsync(
        workspaceSid,
        sid
    );
}
...
| `assignmentStatus` | `List<string>` | Query, Optional | The `assignment_status` of the Tasks you want to read. Can be: `pending`, `reserved`, `assigned`, `canceled`, `wrapping`, or `completed`. Returns all Tasks in the Workspace with the specified `assignment_status`. |
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `Data` property of this instance returns the response data which is of type [Models.ListTaskResponse](../../doc/models/list-task-response.md).
...
string routingTarget = "WKaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
try
{
    ApiResponse<ListTaskResponse> result = await taskrouterV1TaskApi.ListTaskAsync(
        workspaceSid,
        priority,
        assignmentStatus,
        workflowSid,
...
int? timeout = 1;
int? priority = 1;
string taskChannel = "channel";
string workflowSid = "WWaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
string attributes = "{\"body\": \"attributes\"}";
DateTime? virtualStartTime = DateTime.ParseExact("05/14/2014 18:50:02", "yyyy'-'MM'-'dd'T'HH':'mm':'ss.FFFFFFFK",
        provider: CultureInfo.InvariantCulture,
        DateTimeStyles.RoundtripKind);
string routingTarget = "WKaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
string ignoreCapacity = "false";
string taskQueueSid = "WQaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
try
{
    ApiResponse<TaskRouterTask> result = await taskrouterV1TaskApi.CreateTaskAsync(
        workspaceSid,
        timeout,
        priority,
        taskChannel,
